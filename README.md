...
- Optional procedural Web Audio soundscapes
- Split-screen Compare Views mode
- Intro sequence and desktop Explore cursor
- Cinematic image-loading veil
- Live View Transitions support indicator
- Secret fifth destination: **Eclipse**
- Motion toggle, `prefers-reduced-motion` support and instant fallback
- Responsive controls and mobile-focused Compare layout

## Easter egg

Enter the classic sequence `↑ ↑ ↓ ↓ ← → ← → B A`, or tap the **E** monogram five times, to reveal the hidden destination.
...
> Four destinations become one moving canvas. Elsewhere combines shared-element morphs, click-position portal reveals, day/night scene changes, an interactive mini-map, split-screen comparison, optional generative soundscapes and a hidden Eclipse scene using the CSS View Transitions API.
